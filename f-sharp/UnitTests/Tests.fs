...
module GameTests =
    ...
    [<Fact>]
    let ``it should reject guesses that are empty`` () =
        let previousTurn = GameState.newGame "hello"
        let actual = guess (previousTurn) ""
        let expected : GameState = { previousTurn with GuessError = Some GuessError.WrongLength} 
    ...
    let ``it should reject guesses that are too short`` () =
        let previousTurn = GameState.newGame "hello"
        let actual = guess (previousTurn) "1234"
        let expected = { previousTurn with GuessError = Some GuessError.WrongLength} 
    ...
    let ``If the game is ended, further guesses are rejects / state doesn't change`` () = 
        let previousTurn = { GameState.newGame "hello" with Outcome = Some GameOutcome.Lose }
        let actual = guess previousTurn "123456"
    ...
    let ``it should accept guesses that are exactly the expected length`` () =
        ...
        let actual = guess (previousTurn) guessedWord
        let expected = { SecretWord = "hello"; Guesses = [guessedWord]; GuessError = None; Outcome = None} 
    ...
    let ``it should continue the game up to 6 guesses`` () =
        ...
        let mutable previousState = previousTurn 
        ...
        let expected = [0..5] |> List.map (fun turnNumber -> { Secret Guesses = List.replicate turnNumber guessedWord ; GuessError = None; Outcome = None} )
    ...
    let ``The game should end after the 6th guess`` () =
        ...
        let expected = List.concat [
            [0..5] |> List.map (fun turnNumber -> { Guesses = List.replicate turnNumber guessedWord ; GuessError = None } )
